...
public class Player : MonoBehaviour
{
    ...
    public int money = 5000,                 // Divide by 100 to find display value.
                            health = 150,    // 0 - dead , 250 - best health 
                            timeOfDay = 0;   // 0 - morning, 1 - afternoon, 2 - evening
    [SerializeField] private GameObject healthUI, clockUI, moneyUI;
    ...
    {
        if(context.performed && (transform.gameObject.transform.GetChild(0).transform.childCount > 1))
        {
            if (nextAnswer > 0)
            {
                Transform option = transform.GetChild(0).transform.GetChild(selection + 1).transform.GetChild(0);
    ...
    private void changeHealth(int aNum)
    {
        health += aNum;
        healthUI.transform.GetComponent<Heart>().getHealth(health);
    }
    ...
    private void changeMoney(int aNum)
    {
        money += aNum;
