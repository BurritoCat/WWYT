...
public class Dialogue : MonoBehaviour
{
    ...
    [SerializeField] bool needsResponse, needsCharacter;
    int arrayIndex, sentenceIndex;
    bool hasStarted = false;
    SideCharacter thisChar;
    
    void Start()
    {
        thisChar = this.gameObject.GetComponent<SideCharacter>();
        wait = maxWait;
    }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (needsResponse && hasResponse)
                    {
                        ...
                        GameObject.FindWithTag("GameController").GetComponent<PlayerDatabase>().increaseDBIndex();
                        return 0;
                    }

                    //Code for blame choose dialogue
                    if(needsCharacter && hasResponse)
                    {
                        blameCharacterConversation(responseNumbers);
                        return 0;
                    }

                    int[] temp = { 0 };
                    DialogueTreeNode tempNode = thisChar.ConversationNode;
                    textToShow = thisChar.setUpConversation(temp);
                    text.text = "";
                    arrayIndex = 0;
                    sentenceIndex = 0;

                    if(!(tempNode.isConversationEnd))
                        return 0;
    ...
    {
        textToShow = thisChar.setUpConversation(indexes);
        text.text = "";
        arrayIndex = 0;
        sentenceIndex = 0;
    }
    private void blameCharacterConversation(int[] indexes)
    {
        textToShow = thisChar.setUpConversation(indexes[0]);
        text.text = "";
        arrayIndex = 0;
        sentenceIndex = 0;
    }
    ...
    public void changeCharacter(bool aBool)
    {
        needsCharacter = aBool;
    }
}